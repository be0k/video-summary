...
def save_predictions_npz(output_path, predictions):
    """Save visualization-ready predictions as a reusable compressed NPZ."""
    ...
    for key, value in predictions.items():
        ...
        if isinstance(value, np.ndarray):
            ...
            isinstance(x, (str, os.PathLike)) for x in value
        ):
            arrays[key] = np.asarray([str(x) for x in value])
...
def main():
    ...
    parser.add_argument("--save_predictions", type=str, default=None,
                        help="Save raw demo outputs as .npz for later loop-closure correction/export")
    parser.add_argument("--save_pointcloud", "--save_pcl", dest="save_pointcloud",
                        type=str, default=None,
                        help="Save reconstructed map as .pcd, .ply, or .npz after inference")
    parser.add_argument("--pointcloud_source", choices=["depth", "pointmap"], default="depth",
                        help="Point cloud source for --save_pointcloud. depth matches the viewer default.")
    parser.add_argument("--pointcloud_downsample_factor", type=int, default=None,
                        help="Point cloud export downsample; defaults to --downsample_factor")
    parser.add_argument("--pointcloud_max_points", type=int, default=6_000_000,
    ...
    if args.save_predictions:
        save_predictions_npz(args.save_predictions, vis_predictions)
    ...
    if args.save_pointcloud:
        ...
        pc_downsample = (
            ...
            if args.pointcloud_downsample_factor is not None
            else args.downsample_factor
        )
        print(
            f"Saving point cloud to {args.save_pointcloud} "
            f"(source={args.pointcloud_source}, downsample={pc_downsample}, "
            f"max_points={args.pointcloud_max_points})..."
        )
        pointcloud = predictions_to_pointcloud(
            vis_predictions,
            conf_threshold=args.conf_threshold,
            downsample_factor=pc_downsample,
        ...
        save_pointcloud(pointcloud, args.save_pointcloud)
        print(f"Saved point cloud: {args.save_pointcloud} ({len(pointcloud):,} points)")
